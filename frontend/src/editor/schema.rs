...
pub fn default_schema() -> Schema {
    ...
    // Embed: leaf block atom — M-P6. Carries `url`, optional
    // `title`, `provider`, and `height` attributes; rendered as
    // a sandboxed iframe. Default height is provider-specific,
    // resolved at insert time, so default_attrs stays empty.
    nodes.insert(
        NodeType::Embed,
        NodeSpec {
            valid_children: vec![],
    ...
    // #136 — Calendar: container atom holding CalendarEvent children.
    // Marked `atom` so the whole block is selected as one unit
    // (arrow-keys and backspace treat it as indivisible); it's
    // `isolating` so cursor movement can't drift into event
    // internals. Default view/timezone attrs are populated by
    // the insert helper because they depend on user state.
    nodes.insert(
        NodeType::Calendar,
        NodeSpec {
            valid_children: vec![NodeType::CalendarEvent],
            inline_content: false,
            block: true,
            leaf: false,
            code: false,
    ...
    // #136 — CalendarEvent: leaf atom child of Calendar. Carries
    // color / allDay / date-or-timestamp / content. Never
    // insertable at Doc level; the schema enforces that via
    // Calendar being the only valid parent (Doc.valid_children
    // does not include CalendarEvent).
    nodes.insert(
    ...
    // #137 — Kanban board. Container atom (isolating so cursor
    // navigation can't drift into columns) holding KanbanColumn
    // children. Same shape as Calendar: atom + isolating.
    nodes.insert(
        NodeType::Kanban,
        NodeSpec {
            valid_children: vec![NodeType::KanbanColumn],
            inline_content: false,
            block: true,
            leaf: false,
            code: false,
    ...
    // #137 — KanbanColumn: container of cards. Isolating so
    // typing into the title stays inside the column. Not atom
    // — cards are individually selectable.
    nodes.insert(
        NodeType::KanbanColumn,
        NodeSpec {
            valid_children: vec![NodeType::KanbanCard],
            inline_content: false,
            block: true,
            leaf: false,
            code: false,
    ...
    // #137 — KanbanCard: leaf atom card. Only lives inside a
    // KanbanColumn; the schema forbids Doc-level insertion
    // (Doc.valid_children lists Kanban, not KanbanCard).
    nodes.insert(
        NodeType::KanbanCard,
        NodeSpec {
            valid_children: vec![],
    ...
    // - `block: false` because it sits inside a paragraph's
    //   text stream, not between paragraphs.
    // - `leaf: true` + `atom: true` — no children, deletes as a
    //   single position.
    // - `allowed_marks: Some(vec![])` — a mention chip carries
    //   no bold/italic/link on itself; the attrs (user_id +
    //   display) fully describe it.
    // The `Paragraph.content_matches` path already accepts any
    // `NodeType::is_inline() == true` child, so Mention slots
    // in without touching Paragraph's spec.
    nodes.insert(
        NodeType::Mention,
        NodeSpec {
            valid_children: vec![],
    ...
    // #143: sub/superscript are inclusive toggle marks like Bold, but mutually
    // exclusive — a character can't be both.
    marks.insert(
        MarkType::Subscript,
        MarkSpec {
            inclusive: true,
            exclude_all: false,
            excludes: vec![MarkType::Superscript],
        },
    );
    ...
    marks.insert(
        ...
        MarkSpec {
            inclusive: true,
            exclude_all: false,
            excludes: vec![MarkType::Subscript],
        },
    );

    // #148: user @-mention. Behaves like Link — non-inclusive so
    // typing at the trailing edge doesn't extend it, no exclusion
    // rules. Attributes carry `user_id`.
    marks.insert(
        MarkType::Mention,
        MarkSpec {
            inclusive: false,
