...
pub struct ThreadItem {
    ...
    /// Total messages in the thread (opening comment + replies). Default
    /// 0 for older servers that predate the field. The spreadsheet
    /// cell-comment preview shows `message_count - 1` as the reply count.
    #[serde(default)]
...
pub async fn update_thread_anchors(
    ...
    api_patch(
        &format!("/threads/{thread_id}"),
        &serde_json::json!({ "anchorStart": anchor_start, "anchorEnd": anchor_end }),
    )
    .await
}
