...
fn dispatch_paste(
    view: &EditorView,
    txns: &std::rc::Rc<std::cell::RefCell<Vec<Transaction>>>,
    plain: &str,
) {
    dispatch_paste_html(view, txns, plain, "");
}

/// Dispatch a synthetic `paste` event with both text/plain and text/html
/// payloads. Pass `""` for either to omit it.
fn dispatch_paste_html(
    view: &EditorView,
    txns: &std::rc::Rc<std::cell::RefCell<Vec<Transaction>>>,
    plain: &str,
    html: &str,
    ...
    if !plain.is_empty() {
        dt.set_data("text/plain", plain).ok();
    }
    if !html.is_empty() {
        dt.set_data("text/html", html).ok();
    }
...
fn paste_markdown_heading_creates_h1_in_dom() {
    ...
    assert!(html.contains("Hello"),
        "expected heading text 'Hello' in DOM, got: {}", html);
...
fn paste_markdown_bullet_list_creates_ul_in_dom() {
    ...
    let li_count = html.matches("<li").count();
    assert!(li_count >= 2,
        "expected at least 2 <li> elements, got {}: {}", li_count, html);
    assert!(html.contains(">a<") || html.contains(">a</"),
        "expected item text 'a' in DOM: {}", html);
    assert!(html.contains(">b<") || html.contains(">b</"),
        "expected item text 'b' in DOM: {}", html);

    cleanup(&container);
}

/// Regression: pasting markdown source from a source that wraps it in a
/// single `<pre>` (terminals, file viewers) must re-parse the content as
/// markdown rather than land verbatim as one giant CodeBlock.
#[wasm_bindgen_test]
fn paste_pre_wrapped_markdown_with_text_plain_parses_as_markdown() {
    let container = create_container();
    let (view, txns) = create_editor(container.clone(), Node::empty_doc());
    set_cursor(&view, 1);
    ...
    assert!(dom.contains("<ul"),
        "expected <ul> from re-parsed markdown, got: {}", dom);
    let pre_count = dom.matches("<pre").count();
    assert_eq!(pre_count, 0,
        "expected no <pre>/CodeBlock wrapping the pasted markdown, got: {}", dom);

    cleanup(&container);
}

/// Regression: when a source provides only `text/html` as a single `<pre>`
/// wrapper (no `text/plain`), the heuristic must fall back to the
/// CodeBlock's text content and parse it as markdown.
#[wasm_bindgen_test]
fn paste_pre_wrapped_markdown_without_text_plain_parses_as_markdown() {
    let container = create_container();
    let (view, txns) = create_editor(container.clone(), Node::empty_doc());
    set_cursor(&view, 1);
    ...
    assert!(dom.contains("<ul"),
        "expected <ul> from re-parsed CodeBlock text, got: {}", dom);
    let pre_count = dom.matches("<pre").count();
    assert_eq!(pre_count, 0,
        "expected no <pre>/CodeBlock after re-parse, got: {}", dom);

    cleanup(&container);
}

/// Unlabeled code blocks (`<pre><code>…</code></pre>` with no class) must
/// still be preserved as a CodeBlock — re-parsing as markdown would mangle
/// literal code constructs like `*ptr`, `> redirect`, or bare `---`.
#[wasm_bindgen_test]
fn paste_pre_code_without_language_preserves_codeblock() {
    let container = create_container();
    let (view, txns) = create_editor(container.clone(), Node::empty_doc());
    set_cursor(&view, 1);

    let html = "<pre><code># not a heading\n*not emphasis*\n--- not a rule</code></pre>";
    dispatch_paste_html(
        &view,
        &txns,
        "# not a heading\n*not emphasis*\n--- not a rule",
    ...
    assert!(!dom.contains("<h1"),
        "must not interpret '#' inside <pre><code> as a heading, got: {}", dom);
    assert!(!dom.contains("<hr"),
        "must not interpret '---' inside <pre><code> as a horizontal rule, got: {}", dom);
