...
async fn main() {
    ...
    // Fetch pending updates. `replay` is a diagnostic tool an
    // operator runs against the very docs they're investigating —
    // including the over-budget ones the production #91 cap
    // protects the API task from. Pass `usize::MAX` so the cap
    // never fires here; the operator accepts the memory cost.
    println!("Fetching pending updates...");
    let mut updates = match doc_repo.get_pending_updates(&doc_id, usize::MAX).await {
        Ok(u) => u,
        Err(e) => {
    ...
    // Replay must apply updates in clock order to reproduce production
    // state. The storage SK is `UPDATE#<clock>` so the DynamoDB query
    // already returns them clock-ascending — but this is a forensic tool
    // and shouldn't silently depend on that implicit guarantee, so sort
    // explicitly (a no-op when the query order is already correct, and a
    // lexical sort matches the SK's lexical order). (#8)
    updates.sort_by(|a, b| a.clock.cmp(&b.clock));
