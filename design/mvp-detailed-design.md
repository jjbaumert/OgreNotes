...
│
└── infra/                         # AWS CDK (TypeScript) — see infra/README.md
    ├── lib/data.ts                # DynamoDB table + 8 GSIs, S3 + lifecycle, Redis, EFS
    ├── lib/compute.ts             # ECS cluster, api/qdrant/worker, ALB, autoscaling
    ├── lib/dashboard.ts           # CloudWatch SLA dashboard
...
The MVP keymap is listed below. ProseMirror-style alternatives are also bound where they don't conflict.
...
**Message format:** Every WebSocket message is a binary frame: a 1-byte
message type followed by a type-specific payload (yrs binary for the sync
family, UTF-8 JSON for awareness and comment events, raw UTF-8 for ids and
errors). The MVP started with a four-opcode set; the table below reflects
the protocol as it stands after the Phase 2–5 collaboration work and is the
source of truth alongside `crates/collab/src/protocol.rs`:

| Type Byte | Direction | Payload |
|---|---|---|
| `0x00` | Client -> Server | Auth token (UTF-8 string, single-use from `/ws-token`) |
| `0x01` | Bidirectional | Sync step 1: sender's state vector (binary) |
| `0x02` | Bidirectional | Sync step 2: diff computed from the received state vector (binary) |
| `0x03` | Bidirectional | yrs incremental update (binary) |
| `0x04` | Bidirectional | Awareness state — cursor / selection / user info (UTF-8 JSON) |
| `0x05` | Client -> Server | Application-level keepalive (empty payload; defeats ALB idle timeout) |
| `0x06` | Server -> Client | Comment-thread change notification (UTF-8 JSON) |
| `0x07` | Client -> Server | Subscribe to a referenced foreign doc's updates (doc id, UTF-8) |
| `0x08` | Client -> Server | Unsubscribe from a foreign doc (doc id, UTF-8) |
| `0x09` | Server -> Client | Foreign-doc update: 1-byte id length, id bytes, then yrs update bytes |
| `0x0A` | Server -> Client | Awareness departure — a peer disconnected; payload is the leaving user's id (UTF-8). Local-broadcast only (not Redis-fanned) |
| `0xFF` | Server -> Client | Error (UTF-8 string) |

Unknown leading bytes are tolerated: the decoder logs and drops the frame,
so adding a new opcode is backward-compatible with older clients.
