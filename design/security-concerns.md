...
- **Cross-provider account linking** — a user who first signed up through one OAuth provider (e.g. GitHub) and later signs in through another (e.g. Google) is logged into the *same* account when the email matches, rather than getting a duplicate. This is deliberately gated on a **verified** email from *both* providers: GitHub emails are resolved from `/user/emails` and required to be verified, and Google's `email_verified` claim must be `true`. Because an unverified address can never reach the linking path, an attacker cannot register a victim's email on a second provider to adopt the existing account — the link is not a takeover oracle. Only self-service OAuth providers link this way; SAML/SSO accounts stay SSO-only and dev-login can never adopt a real account. Each link is recorded as a distinct `SecurityAudit` `AccountLinked { from_provider, to_provider }` event so the merge is never invisible behind a plain `LoginSuccess`.
